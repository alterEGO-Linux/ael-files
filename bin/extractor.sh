#!/usr/bin/env sh
# ------------------------------------------------------------------------ INFO
# [.ael/bin/extractor.sh]
# author        : Pascal Malouin (https://github.com/alterEGO-Linux)
# created       : 2023-08-01 19:55:12 UTC
# updated       : 2026-04-09 10:45:25 UTC
    ...
    if [ -f "$file" ]; then
        ...
        case "$file" in
            ...
            *.tar.gz|*.tgz)
                ...
                printf "${__blue}[-]${__reset} extractor: '%s' extracted.\n" "$file"
                ;;
            *.tar.bz2|*.tbz2)
                check_applications tar || exit 1
                mkdir -p "$directory"
            ...
            *.iso)
                ...
                mkdir -p "$directory"
                7z x -- "$file" -o"$directory"
                printf "${__blue}[-]${__reset} extractor: '%s' extracted (ISO image).\n" "$file"
                ;;
            *.cpio)
                check_applications cpio || exit 1
                mkdir -p "$directory"
                ( cd "$directory" && cpio -id < "$file" )
                printf "${__blue}[-]${__reset} extractor: '%s' extracted (cpio archive).\n" "$file"
                ;;

            # --| fallback
            *)
                printf '%s\n' "${__red}[!]${__reset} extractor: '$file' - unknown archive method."
                ;;
        esac
    else
        printf "${__red}[!]${__reset} extractor: '$file' is not a valid file."
    fi
done
